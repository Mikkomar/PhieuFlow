...
| Path | Purpose |
| --- | --- |
| `Infrastructure/AppHostFixture.cs` | Starts the AppHost graph (SQL, migrations, seed, hub, form-builder) once for the assembly; launches one headless Chromium. |
...
| `Auth/` | ADR 0005 — hub rejects unauthenticated / wrong-scope callers. **Skipped.** |

## Running

Requires Docker. First run installs Chromium automatically.
...
3. Replace any placeholder in the body — `Fixture.FormFillerBaseUrl`, the
   `/forms/{id}/submissions` calls, `GetTokenAsync` — with the real endpoint/flow.
